...
/**
 * Extract interactive list/button reply from webhook payload (multiple shapes).
 */
export function extractInteractiveReply(body) {
    // 1. Meta WhatsApp Cloud API format
    if (body?.interactive?.list_reply?.id) {
        return body.interactive.list_reply
    }
    if (body?.interactive?.button_reply?.id) {
        return body.interactive.button_reply
    }

    // 2. Interakt format (JSON STRING inside body.data.message.message)
    let parsedMsg = null
    try {
    ...
    const candidates = [
        parsedMsg?.list_reply,
        parsedMsg?.button_reply,
...
export function parseVisitRating(body, textStr = '') {
    const listReply = extractInteractiveReply(body)
    if (listReply?.id) {
        const idTrimmed = String(listReply.id).trim()
        
        // Match explicit Rating IDs (R1 to R5)
        const rMatch = idTrimmed.match(/^R([1-5])$/i)
        if (rMatch) return parseInt(rMatch[1])

        // If it's a raw number 1-5 from a list reply, ONLY accept it if the title clearly looks like a rating
        const numMatch = idTrimmed.match(/^[1-5]$/)
        if (numMatch) {
            const title = String(listReply.title || '').toLowerCase()
            if (title.includes('star') || title.includes('⭐') || title.includes('excellent') || title.includes('good') || title.includes('average') || title.includes('poor')) {
                return parseInt(numMatch[0])
            }
            // Otherwise, it's likely a language selection (like "5 Punjabi"), ignore it.
            return null
        }
    }
