...
function MarkerWithPopup({ salon, isSelected }) {
  ...
  return (
    <Marker 
      position={[parseFloat(salon.lat), parseFloat(salon.lng)]}
      icon={customIcon}
      ref={markerRef}
    >
      <Popup>
        <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>{salon.name}</div>
        <div style={{ fontSize: '12px', color: '#4b5563' }}>{salon.specialty}</div>
      </Popup>
    </Marker>
  )
}

export default function LeafletMapComponent({ salons, selectedsalon, mapKey, children }) {
  const mapCenter = selectedsalon?.lat && selectedsalon?.lng 
    ? [parseFloat(selectedsalon.lat), parseFloat(selectedsalon.lng)]
    : salons.length > 0 && salons[0].lat && salons[0].lng
    ? [parseFloat(salons[0].lat), parseFloat(salons[0].lng)]
    : [20.5937, 78.9629]; // Default to India

  const mapZoom = selectedsalon ? 15 : salons.length > 0 ? 12 : 5;
    ...
    <div style={{ width: '100%', height: '100%', position: 'relative', zIndex: 1 }}>
      ...
      >
        ...
        {salons.map(salon => {
          ...
          const isSelected = selectedsalon && (selectedsalon.id === salon.id || selectedsalon.code === salon.code);
          return (
            <MarkerWithPopup 
