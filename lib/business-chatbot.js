...
import { joinQueue } from './queue-manager'
...
export async function handleBusinessMessage(senderPhone, messageText, session) {
    ...
    if (currentState === 'qr_join_language') {
        ...
        // Call the internal queue manager to reuse all existing logic safely
        try {
...
5. If they want to join the queue, tell them to type "JOIN [CODE]", scan the QR code at the premises, or search for us on https://tokenpe.online/find. Do NOT try to join them yourself. (Their code is: ${business.code})`
