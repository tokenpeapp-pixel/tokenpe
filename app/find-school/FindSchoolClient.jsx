...
import {
  MapPin, Search, Clock, Users, Link as LinkIcon, School, ArrowRight, Star, Book, GraduationCap
} from 'lucide-react'
...
function SchoolCard({ school, isNearby, onClick, isSelected }) {
  const waLink = `https://wa.me/${WA_NUMBER}?text=JOIN%20${encodeURIComponent(school.code)}`
  ...
  return (
    ...
    >
      <div>
        <h3 className="cc-title">{school.name}</h3>
        <div className="cc-meta">
          <span className="cc-meta-item"><Book size={14} /> {school.specialty || 'General Administration'}</span>
          {school.city && <span className="cc-meta-item"><MapPin size={14} /> {school.city}</span>}
          {isNearby && school.distance_km && (
            <span className="cc-meta-item" style={{color: '#3b82f6'}}><MapPin size={14} /> {school.distance_km} km</span>
          )}
        </div>
      </div>
      
      <div className="cc-right">
        {school.is_closed_today ? (
          ...
          <div style={{ textAlign: "right", display: "flex", flexDirection: "column", gap: "2px" }}>
            ...
            <div className="cc-wait-val" style={{color: '#3b82f6'}}>
              <Clock size={16} /> {Math.max((school.waiting_count || 0) * 5, 10)} mins
            </div>
            <div style={{ fontSize: "11px", color: "#64748b" }}>
              {school.waiting_count || 0} waiting
            </div>
          </div>
        )}
    ...
    <>
      <div className="empty-state-overlay">
        ...
        <p style={{ fontSize: 13, color: '#94a3b8', textAlign: 'center', marginBottom: 16, maxWidth: 280, lineHeight: 1.5 }}>
          Try searching for a different location or invite your institution's admin to join TokenPe!
        </p>
        <button className="btn-invite" onClick={() => setShowPopup(true)}>Invite Admin</button>
      </div>
      ...
      {showPopup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ background: "#0f172a", border: "1px solid rgba(59,130,246,0.2)", borderRadius: "16px", padding: "32px", width: "90%", maxWidth: "400px", position: "relative", textAlign: "center", boxShadow: "0 20px 40px rgba(0,0,0,0.5)" }}>
            <button 
            ...
            <p style={{ fontSize: "14px", color: "#bfdbfe", marginBottom: "24px", lineHeight: 1.5 }}>
              Share this link with your school admin so they can register for free and set up digital queuing.
            </p>
...
export default function FindSchoolClient({ initialSchools, initialQ, initialCity, initialSpecialty, cities, specialties }) {
  const router = useRouter()
  const [query, setQuery] = useState(initialQ || '')
  const [selectedCity, setSelectedCity] = useState(initialCity || '')
  const [selectedSpecialty, setSelectedSpecialty] = useState(initialSpecialty || '')
  const [schools, setClinics] = useState(initialSchools || [])
  ...
  const displayedSchools = isNearbyMode 
    ? nearbySchools.filter(c => {
  ...
  const fetchSchools = useCallback(async (q, city, spec, status) => {
    setLoading(true)
    setIsNearbyMode(false)
    setGpsError('')
    try {
      const params = new URLSearchParams()
      params.set('vertical', 'school')   // ← strict vertical isolation
      if (q) params.set('q', q)
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      fetchSchools(query, selectedCity, selectedSpecialty, statusFilter)
      const params = new URLSearchParams()
      if (query) params.set('q', query)
      if (selectedCity) params.set('city', selectedCity)
      if (selectedSpecialty) params.set('specialty', selectedSpecialty)
      router.replace(`/find-school${params.toString() ? '?' + params.toString() : ''}`, { scroll: false })
    }, 300)
    return () => clearTimeout(debounceRef.current)
  }, [query, selectedCity, selectedSpecialty, statusFilter, fetchSchools, router])
  ...
  const fetchNearby = useCallback(async (lat, lng) => {
    try {
      ...
      setNearbySchools([])
      setGpsError('Could not fetch nearby schools. Please try again.')
    } finally {
      setGpsLoading(false)
    }
  }, [])
  ...
  const allSpecialties = specialties && specialties.length ? specialties : ['General Administration', 'Admissions', 'Fee Collection', 'Registrar', 'Principal']
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .find-nav { padding: 20px 24px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(59,130,246,0.1); position: sticky; top: 0; z-index: 100; background: #020617; }
        ...
        .btn-search { padding: 0 32px; height: 53px; background: #3b82f6; color: #fff; border: none; border-radius: 12px; font-weight: 700; font-size: 16px; cursor: pointer; transition: background 0.2s; }
        ...
        .btn-near-me.active { background: rgba(59,130,246,0.1); border-color: #3b82f6; color: #3b82f6; }
        ...
        .school-list { display: flex; flex-direction: column; gap: 16px; padding-bottom: 40px; }
        
        /* Clinic Card */
        .school-card { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 16px; padding: 24px; display: flex; justify-content: space-between; alignItems: center; flex-wrap: wrap; gap: 16px; transition: all 0.2s; cursor: pointer; }
        .school-card:hover { border-color: rgba(59,130,246,0.3); background: rgba(255,255,255,0.04); }
        .school-card.selected { border-color: #3b82f6; background: rgba(59,130,246,0.05); }
        ...
        .cc-btn-active { background: #3b82f6; color: #fff; }
        .cc-btn-active:hover { background: #60a5fa; }
        ...
        .empty-icon-wrap { width: 64px; height: 64px; background: rgba(59,130,246,0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 20px; }
        .btn-invite { background: transparent; border: 1px solid #3b82f6; color: #3b82f6; padding: 10px 24px; border-radius: 8px; font-size: 13px; font-weight: 700; cursor: pointer; margin-top: 8px; }
      ...
      <nav className="find-nav">
        ...
        <a href="/school-login" className="find-nav-btn">
          School Login
        </a>
      </nav>
      ...
      <div className={`layout-container ${activeView === 'map' ? 'map-active' : ''}`}>
        <div className="left-col">
          <h1 className="page-title">Find a TokenPe School</h1>
          <p className="page-sub">Search by school name, department, or city to join a waitlist remotely.</p>
          ...
          <div className="search-row">
            <div className="search-input-wrap">
              ...
              <input 
                ...
                type="text" 
                placeholder="Search e.g., 'Engineering in Mumbai'" 
                value={query} 
          ...
          <div className="mobile-map-container" style={{ display: activeView === 'map' ? 'block' : 'none' }}>
            {activeView === 'map' && (
              <MapComponent schools={displayedSchools} selectedSchool={selectedSchool} mapKey={mapMountKey}>
                {displayedSchools.length === 0 && !loading && !gpsLoading && <EmptyState />}
              </MapComponent>
            )}
          </div>
          ...
          <div className="filters-row">
            ...
            >
              <MapPin size={14} color={isNearbyMode ? "#3b82f6" : "#64748b"} /> 
            ...
            {(loading || gpsLoading) ? (
              ...
              <>
                <h2 style={{ fontSize: "20px", fontWeight: 600, marginBottom: "8px", color: "#f8fafc" }}>
                  {displayedSchools.length} {displayedSchools.length === 1 ? "Result" : "Results"} Found
                </h2>
                {displayedSchools.map(school => (
                  <SchoolCard 
                    ...
                    isSelected={selectedSchool && (selectedSchool.id === school.id || selectedSchool.code === school.code)}
                    onClick={() => {
        ...
        <div className="right-col">
          <MapComponent schools={displayedSchools} selectedSchool={selectedSchool} mapKey={0}>
            {displayedSchools.length === 0 && !loading && !gpsLoading && <EmptyState />}
          </MapComponent>
        </div>
      </div>
