...
import { handleBusinessMessage } from './business-chatbot'
...
export async function handleIncomingMessage(senderPhone, messageText, rawPayload) {
    ...
    // ==========================================
    // BUSINESS BOT ROUTING
    // ==========================================
    if (text.toLowerCase().startsWith('join ') || session?.data?.mode === 'clinic' || session?.data?.mode === 'business') {
        return await handleBusinessMessage(senderPhone, messageText, { ...session, data: sessionData })
    }
