...
// ── Send Interactive Rating List Message via Interakt ──────────────────────
export async function sendInteractiveRating(phone, clinicName) {
    ...
    try {
        const res = await fetch('https://api.interakt.ai/v1/public/message/', {
            ...
            body: JSON.stringify({
                ...
                data: {
                    message: {
                        type: 'list',
                        header: 'Rate Your Visit',
                        body: `How was your consultation at ${clinicName}? Please rate your experience so we can improve.`,
                        footer: 'TokenPe',
                        action: {
                            button: 'Tap to Rate ⭐',
                            sections: [
                                {
                                    title: 'Select Rating (1 to 5)',
                                    rows: [
                                        { id: 'rate_5', title: '⭐⭐⭐⭐⭐', description: 'Excellent' },
                                        { id: 'rate_4', title: '⭐⭐⭐⭐', description: 'Good' },
                                        { id: 'rate_3', title: '⭐⭐⭐', description: 'Average' },
                                        { id: 'rate_2', title: '⭐⭐', description: 'Fair' },
        ...
        const data = await res.json()
        console.log(`[Interactive Rating] → +91${phoneNumber}:`, JSON.stringify(data))
    } catch (err) {
        console.error('[Interactive Rating] Error:', err.message)
    }
}
