...
function RestaurantCard({ restaurant, isNearby, onClick, isSelected }) {
  const waLink = `https://wa.me/${WA_NUMBER}?text=JOIN%20${encodeURIComponent(restaurant.code)}`
  ...
  return (
    ...
    >
      <div>
        <h3 className="cc-title">{restaurant.name}</h3>
        <div className="cc-meta">
          <span className="cc-meta-item"><Utensils size={14} /> {restaurant.specialty || 'Casual Dining'}</span>
          {restaurant.city && <span className="cc-meta-item"><MapPin size={14} /> {restaurant.city}</span>}
          {isNearby && restaurant.distance_km && (
            <span className="cc-meta-item" style={{color: '#d4af37'}}><MapPin size={14} /> {restaurant.distance_km} km</span>
          )}
        </div>
      </div>
      
      <div className="cc-right">
        {restaurant.is_closed_today ? (
        ...
        ) : restaurant.queue_paused ? (
          <div style={{ textAlign: "right" }}>
            <div className="cc-wait-label">Status</div>
            <div className="cc-wait-val" style={{color: '#b8860b'}}>
          ...
          <div style={{ textAlign: "right", display: "flex", flexDirection: "column", gap: "2px" }}>
            ...
            <div className="cc-wait-val" style={{color: '#d4af37'}}>
              <Clock size={16} /> {Math.max((restaurant.waiting_count || 0) * 5, 10)} mins
            </div>
            <div style={{ fontSize: "11px", color: "#64748b" }}>
              {restaurant.waiting_count || 0} waiting
            </div>
          </div>
        )}
    ...
    <>
      <div className="empty-state-overlay">
        ...
        <p style={{ fontSize: 13, color: '#94a3b8', textAlign: 'center', marginBottom: 16, maxWidth: 280, lineHeight: 1.5 }}>
          Try searching for a different location or invite the restaurant manager to join TokenPe!
        </p>
      ...
      {showPopup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ background: "#2e0a0a", border: "1px solid rgba(212,175,55,0.2)", borderRadius: "16px", padding: "32px", width: "90%", maxWidth: "400px", position: "relative", textAlign: "center", boxShadow: "0 20px 40px rgba(0,0,0,0.5)" }}>
            <button 
            ...
            <p style={{ fontSize: "14px", color: "#fed7aa", marginBottom: "24px", lineHeight: 1.5 }}>
              Share this link with your restaurant manager so they can register for free and set up digital queuing.
            </p>
            <div style={{ display: "flex", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 8px 8px 16px", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.1)" }}>
              ...
              <button 
                onClick={handleCopy}
                style={{ background: "#b8860b", color: "#fff", border: "none", borderRadius: "8px", padding: "10px 20px", fontSize: "14px", fontWeight: 700, cursor: "pointer", transition: "all 0.2s", opacity: copied ? 0.8 : 1 }}
              >
...
export default function FindRestaurantClient({ initialClinics, initialQ, initialCity, initialSpecialty, cities, specialties }) {
  ...
  const fetchClinics = useCallback(async (q, city, spec, status) => {
    ...
    try {
      ...
      params.set('vertical', 'restaurant')   // ← strict vertical isolation
      if (q) params.set('q', q)
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      router.replace(`/find-restaurant${params.toString() ? '?' + params.toString() : ''}`, { scroll: false })
    }, 300)
  ...
  const fetchNearby = useCallback(async (lat, lng) => {
    try {
      const res = await fetch(`/api/clinics/nearby?lat=${lat}&lng=${lng}&radius=50000&vertical=restaurant`)
      const json = await res.json()
      setNearbyClinics(json.clinics || [])
      setIsNearbyMode(true)
    } catch {
      setNearbyClinics([])
      setGpsError('Could not fetch nearby restaurants. Please try again.')
    } finally {
      setGpsLoading(false)
    }
  }, [])
  ...
  const allSpecialties = specialties && specialties.length ? specialties : ['Fine Dining', 'Casual Dining', 'Cafe', 'Buffet', 'Bar']
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Plus Jakarta Sans', sans-serif; background: #1c0505; color: #f8fafc; }
        
        .find-nav { padding: 20px 24px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(212,175,55,0.1); position: sticky; top: 0; z-index: 100; background: #1c0505; }
        ...
        .btn-search { padding: 0 32px; height: 53px; background: #d4af37; color: #fff; border: none; border-radius: 12px; font-weight: 700; font-size: 16px; cursor: pointer; transition: background 0.2s; }
        .btn-search:hover { background: #e5c158; }
        ...
        .btn-near-me.active { background: rgba(212,175,55,0.1); border-color: #d4af37; color: #d4af37; }
        ...
        .filter-select option { background: #2e0a0a; color: #fff; }
        ...
        .clinic-card:hover { border-color: rgba(212,175,55,0.3); background: rgba(255,255,255,0.04); }
        .clinic-card.selected { border-color: #d4af37; background: rgba(212,175,55,0.05); }
        ...
        .cc-btn-active { background: #d4af37; color: #fff; }
        .cc-btn-active:hover { background: #e5c158; }
        .cc-btn-disabled { background: rgba(255,255,255,0.1); color: #64748b; cursor: not-allowed; }
        
        .empty-state-overlay { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(28,5,5,0.85); padding: 32px; border-radius: 20px; backdrop-filter: blur(8px); display: flex; flex-direction: column; align-items: center; width: 340px; text-align: center; z-index: 10; border: 1px solid rgba(255,255,255,0.1); }
        .empty-icon-wrap { width: 64px; height: 64px; background: rgba(212,175,55,0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 20px; }
        .btn-invite { background: transparent; border: 1px solid #d4af37; color: #d4af37; padding: 10px 24px; border-radius: 8px; font-size: 13px; font-weight: 700; cursor: pointer; margin-top: 8px; }
      ...
      <nav className="find-nav">
        ...
        <a href="/restaurant-login" className="find-nav-btn">
          Restaurant Login
        </a>
      </nav>
      ...
      <div className={`layout-container ${activeView === 'map' ? 'map-active' : ''}`}>
        <div className="left-col">
          <h1 className="page-title">Find a TokenPe Restaurant</h1>
          <p className="page-sub">Search by restaurant name, cuisine, or city to join a waitlist remotely.</p>
          ...
          <div className="search-row">
            <div className="search-input-wrap">
              ...
              <input 
                ...
                type="text" 
                placeholder="Search e.g., 'Cafe in Mumbai'" 
                value={query} 
          ...
          <div className="filters-row">
            ...
            >
              <MapPin size={14} color={isNearbyMode ? "#d4af37" : "#64748b"} /> 
          ...
          <div className="clinic-list">
            {(loading || gpsLoading) ? (
              ...
              <>
                ...
                {displayedClinics.map(restaurant => (
                  <RestaurantCard 
                    key={restaurant.id || restaurant.code} 
                    restaurant={restaurant} 
                    isNearby={isNearbyMode} 
                    isSelected={selectedClinic && (selectedClinic.id === restaurant.id || selectedClinic.code === restaurant.code)}
                    onClick={() => {
                      setSelectedClinic(restaurant)
