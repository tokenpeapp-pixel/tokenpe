...
export default function LoginPage() {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (errType === 'no_clinic') {
                setError('No salon account found for this Google email. Please register first, or use your Salon Code to log in.')
                setMode('register')
    ...
    const [regServiceType, setRegServiceType] = useState('Unisex Salon')
    ...
    async function handleRegister(e) {
        ...
        if (!regLat || !regLng) {
            setError('Salon GPS location is mandatory. Please click "Detect GPS Location" and allow permissions.')
            setLoading(false)
            return
        }
        ...
        try {
            const res = await fetch('/api/auth/register', {
                ...
                body: JSON.stringify({ 
                    ...
                    email: regEmail, 
                    pin: regPin,
                    specialty: regServiceType === 'Other' ? customServiceType : regServiceType,
                    city: regCity,
                    lat: regLat,
                    lng: regLng
                })
    ...
    if (celebration) {
        return <CelebrationScreen clinicName={celebration.clinicName} trialEnd={celebration.trialEnd} onDone={() => router.push('/salon-dashboard')} />
    }
    ...
    return (
        <div className="flex min-h-screen w-full bg-[#FCFCFA] overflow-hidden font-sans text-slate-800" suppressHydrationWarning={true}>
            ...
            <div className="relative hidden md:flex flex-col justify-between w-[48%] min-h-screen bg-gradient-to-br from-[#2a1115] to-[#4a1c24] text-white p-12 overflow-hidden z-10">
                ...
                <div className="absolute inset-0 z-0 pointer-events-none">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-b from-[#2a1115]/60 to-[#2a1115]/95" />
                </div>
                ...
                <div className="absolute top-0 left-0 w-full h-full z-0 overflow-hidden pointer-events-none">
                    ...
                    <div className="absolute -top-40 -left-40 w-96 h-96 bg-rose-400 rounded-full mix-blend-screen filter blur-[100px] opacity-25 animate-pulse" />
                </div>
                ...
                <div className="relative z-10 flex flex-col items-start max-w-lg">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex items-center cursor-pointer mb-8" onClick={() => router.push('/salons')}>
                    ...
                    <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-3xl lg:text-4xl font-bold leading-tight mb-6">
                        Smart Queue<br />
                        Management for<br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-rose-200">Modern Salons</span>
                    </motion.h1>

                    <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-rose-50/80 text-base mb-8 max-w-md">
                        Digital queue management that reduces wait times, keeps clients happy, and improves salon experience.
                    </motion.p>
                    ...
                    <div className="flex flex-col gap-4 w-full">
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}>
                            <div className="flex items-center gap-4 bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl shadow-lg cursor-default hover:-translate-y-1 hover:bg-white/10 hover:shadow-xl transition-all duration-200">
                                <div className="flex-shrink-0 w-8 h-8 rounded-full bg-rose-500/20 flex items-center justify-center text-rose-300"><Mic className="w-5 h-5" /></div>
                                <div>
                                    <div className="font-semibold text-white">Voice updates in 10 Indian languages</div>
                                    <div className="text-sm text-rose-100/60 mt-0.5">Inclusive. Accessible. Effortless.</div>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
                            <div className="flex items-center gap-4 bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl shadow-lg cursor-default hover:-translate-y-1 hover:bg-white/10 hover:shadow-xl transition-all duration-200">
                                <div className="flex-shrink-0 w-8 h-8 rounded-full bg-rose-500/20 flex items-center justify-center text-rose-300"><Bell className="w-5 h-5" /></div>
                                <div>
                                    <div className="font-semibold text-white">Real-time notifications</div>
                                    <div className="text-sm text-rose-100/60 mt-0.5">Stay informed. Always.</div>
                                </div>
                            </div>
                        </motion.div>
                    </div>
                ...
                <div className="relative z-10 flex items-center gap-2 text-rose-200/50 mt-12">
                    <ShieldCheck className="w-4 h-4" />
                    <span className="text-sm">Secure. Reliable. Built for Salons.</span>
                </div>
            </div>

            {/* RIGHT PANEL (approx 52%) */}
            <div className="relative flex-1 min-h-screen flex flex-col md:flex-row items-center justify-center p-0 md:p-6 lg:p-12 z-10 bg-gradient-to-br from-[#2a1115] to-[#4a1c24] md:bg-none md:bg-transparent">
                ...
                <div className="hidden md:block absolute top-0 right-0 w-[500px] h-[500px] bg-rose-200/15 rounded-full mix-blend-multiply filter blur-[100px] pointer-events-none" />
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="relative z-10 w-full md:max-w-[440px] bg-white md:bg-white/70 md:backdrop-blur-xl rounded-t-[40px] md:rounded-2xl shadow-[0_-10px_40px_rgba(0,0,0,0.1)] md:shadow-[0_25px_80px_rgba(0,0,0,0.08)] md:border border-white/60 px-6 py-8 pt-10 md:p-8 flex-1 md:flex-none">
                    ...
                    <button onClick={() => router.push('/salons')} className="flex items-center gap-2 text-slate-500 hover:text-slate-800 transition-colors font-medium text-sm mb-8">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
                        Back to Home
                    </button>

                    <h2 className="hidden md:block text-2xl font-bold text-slate-900 tracking-tight mb-2">
                        {mode === 'login' ? 'Welcome back!' : 'Create New Salon'}
                    </h2>
                    <p className="hidden md:block text-slate-500 mb-8">
                        {mode === 'login' ? 'Sign in to your TokenPe dashboard' : 'Register your salon and get started in 2 minutes'}
                    </p>
                    ...
                    <div className="relative flex bg-slate-100/50 p-1.5 rounded-2xl mb-8 border border-slate-200/50">
                        ...
                        <div className="flex-1 relative z-10">
                            ...
                            >
                                <Building className="w-4 h-4" /> New Salon
                            </button>
                        </div>
                        {/* Active Pill Background */}
                        <div className="absolute top-1.5 bottom-1.5 left-1.5 right-1.5 flex pointer-events-none">
                            <motion.div 
                                className="w-1/2 h-full bg-rose-600 rounded-xl shadow-sm"
                        ...
                        {mode === 'verify-otp' && (
                            <motion.form key="verify-otp" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleResetPin} className="space-y-5">
                                ...
                                <div className="flex justify-center gap-6 mt-4">
                                    <button type="button" onClick={handleSendOtp} className="text-sm font-semibold text-rose-600 hover:text-rose-700">Resend OTP</button>
                                    <button type="button" onClick={() => { setMode('login'); setError(''); setSuccess(''); }} className="text-sm font-semibold text-slate-500 hover:text-slate-700">← Back to Login</button>
                                </div>
                            </motion.form>
                        )}
                            ...
                            <motion.form key="register" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleRegister} className="space-y-5 h-[65vh] md:h-[55vh] overflow-y-auto pr-2 pb-4 scrollbar-hide">
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input value={regName} onChange={e => setRegName(e.target.value)} placeholder="Glamour Salon" required className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 focus:bg-white transition-all shadow-inner" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Salon Type *</label>
                                    <div className="relative">
                                        <Activity className="absolute left-3 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400" />
                                        <select value={regServiceType} onChange={e => setRegServiceType(e.target.value)} required className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 pl-10 pr-10 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 focus:bg-white transition-all shadow-inner appearance-none bg-[url('data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2024%2024%22%20stroke%3D%22%2364748b%22%20stroke-width%3D%222.5%22%3E%3Cpath%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20d%3D%22M19.5%208.25l-7.5%207.5-7.5-7.5%22%2F%3E%3C%2Fsvg%3E')] bg-no-repeat bg-[position:right_14px_center] bg-[length:14px]">
                                ...
                                {regServiceType === 'Other' && (
                                    <div>
                                        <label className="block text-sm font-semibold text-slate-700 mb-2">Enter Custom Salon Type *</label>
                                        <input value={customServiceType} onChange={e => setCustomServiceType(e.target.value)} placeholder="e.g. Nails, Eyelashes, etc." required className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-4 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 focus:bg-white transition-all shadow-inner" />
                                    </div>
                                )}
                                <div>
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Salon GPS Location</label>
                    ...
                    <div className="mt-8 flex items-center justify-center gap-2 text-rose-600/70 text-xs font-semibold">
