...
// ── Get the MSG91-ready 12-digit number ──────────────────────────────────────
function msg91Phone(phone) {
...
// ── Send text message via MSG91 ───────────────────────────────────────────
export async function sendText(phone, message) {
    if (!MSG91_AUTHKEY) {
        console.warn('[MSG91] API key not configured — skipping text')
        return
    }
    const phoneNumber = msg91Phone(phone)
    try {
        const payload = {
        ...
        console.log(`[Text] → +${phoneNumber}:`, JSON.stringify(data))
    } catch (err) {
...
export async function sendTemplateMessage({ phone, templateName, languageCode = 'en', bodyValues = [], headerValues = [], callbackData = 'tokenpe_broadcast' }) {
    if (!MSG91_AUTHKEY) {
        console.warn(`[MSG91] API key not configured — skipping template ${templateName}`)
        return
    }
    const phoneNumber = msg91Phone(phone)
...
// ── Send image message via MSG91 ──────────────────────────────────────────
export async function sendImage(phone, mediaUrl, caption = '') {
    if (!MSG91_AUTHKEY) {
        console.warn('[MSG91] API key not configured — skipping image')
        return
    }
    const phoneNumber = msg91Phone(phone)
    try {
        ...
        const data = await res.json()
        console.log(`[Image] → +${phoneNumber}:`, JSON.stringify(data))
    } catch (err) {
        console.error('[Image] Error:', err.message)
    }
}

// ── Send Interactive Rating Message via MSG91 ──────────────────────────
export async function sendInteractiveRating(phone, clinicName, language = 'en') {
    if (!MSG91_AUTHKEY) {
        console.warn('[MSG91] API key not configured — skipping interactive rating')
        return
    }
    const phoneNumber = msg91Phone(phone)
    try {
        ...
        const data = await res.json()
        console.log(`[Rating Interactive] → +${phoneNumber}:`, JSON.stringify(data))
...
// ── Send CRM Interactive Rating Message via MSG91 ───────────────────────
export async function sendCRMInteractiveRating(phone, clinicName, language = 'en') {
    if (!MSG91_AUTHKEY) {
        console.warn('[MSG91] API key not configured — skipping CRM rating')
        return
    }
    const phoneNumber = msg91Phone(phone)
    try {
        ...
        const data = await res.json()
        console.log(`[CRM Rating Interactive] → +${phoneNumber}:`, JSON.stringify(data))
...
export async function sendInteractiveList(phone, headerText, bodyText, buttonText, sections) {
    if (!MSG91_AUTHKEY) {
        console.warn('[MSG91] API key not configured — skipping interactive list')
        return false
    }
    const phoneNumber = msg91Phone(phone)
    try {
...
// ── Send Interactive Button via MSG91 ─────────────────────────────────────
export async function sendInteractiveButton(phone, bodyText, buttons) {
    if (!MSG91_AUTHKEY) {
        console.warn('[MSG91] API key not configured — skipping interactive button')
        return false
    }
    const phoneNumber = msg91Phone(phone)
    try {
...
// ── Generate voice note + upload to Supabase + send via MSG91 ─────────────
...
export async function sendVoice({ phone, language, event, token, position, clinicName, currentToken }) {
    ...
    // 4. Send via MSG91 as audio message
    if (!MSG91_AUTHKEY) {
        console.warn('[Voice] MSG91_AUTHKEY not set — skipping MSG91 send')
        return
    }
    ...
    console.log(`[Voice Audio] → +${phoneNumber}:`, JSON.stringify(result))
}
...
// ── Send Interactive List via MSG91 ──────────────────────────────────────────
export async function sendInteractiveList(phone, headerText, bodyText, buttonLabel, sections) {
    if (!process.env.MSG91_AUTHKEY) {
        console.warn('[MSG91] API key not configured, skipping interactive list')
        return
    }
    const phoneNumber = cleanPhone(phone)
    const p = phoneNumber.startsWith('91') ? phoneNumber : '91' + phoneNumber
    try {
        const payload = {
            "integrated-number": process.env.MSG91_INTEGRATED_NUMBER,
            "content_type": "interactive",
            "payload": {
                "messaging_product": "whatsapp",
                "to": p,
        ...
        if (!res.ok) {
            ...
            console.error('[MSG91] List failed:', res.status, txt)
        } else {
            console.log(`[MSG91] Interactive List sent to ${p}`)
        }
    } catch (err) {
        console.error('[MSG91] Network error sending list:', err)
    }
}
