...
export async function sendInteractiveRating(phone, clinicName) {
    ...
    const phoneNumber = interaktPhone(phone)
    try {
        const ratingMsg = `🌟 *Rate Your Visit at ${clinicName}*

How was your consultation? Please reply with a number:
...
_Powered by TokenPe_`
        ...
        const res = await fetch('https://api.interakt.ai/v1/public/message/', {
            ...
            body: JSON.stringify({
                ...
                type: 'Text',
                data: { message: ratingMsg }
            })
        })
        const data = await res.json()
        console.log(`[Rating] → +91${phoneNumber}:`, JSON.stringify(data))
    } catch (err) {
...
export async function sendCRMInteractiveRating(phone, clinicName) {
    ...
    const phoneNumber = interaktPhone(phone)
    try {
        const ratingMsg = `🌟 *Feedback for ${clinicName}*

Was this message helpful? Please reply with a code:
...
You can also add text after the code (e.g. "C5 Very helpful!")
...
_Powered by TokenPe CRM_`
        ...
        const data = await res.json()
        console.log(`[CRM Rating] → +91${phoneNumber}:`, JSON.stringify(data))
    } catch (err) {
        console.error('[CRM Rating] Error:', err.message)
    }
}
