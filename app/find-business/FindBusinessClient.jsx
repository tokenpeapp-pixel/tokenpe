...
import {
  MapPin, Search, Clock, Users, Link as LinkIcon, Briefcase, ArrowRight, Star, Building, ClipboardList
} from 'lucide-react'
...
function BusinessCard({ business, isNearby, onClick, isSelected }) {
  const waLink = `https://wa.me/${WA_NUMBER}?text=JOIN%20${encodeURIComponent(business.code)}`
  ...
  return (
    ...
    >
      <div>
        <h3 className="cc-title">{business.name}</h3>
        <div className="cc-meta">
          <span className="cc-meta-item"><Building size={14} /> {business.specialty || 'General Management'}</span>
          {business.city && <span className="cc-meta-item"><MapPin size={14} /> {business.city}</span>}
          {isNearby && business.distance_km && (
            <span className="cc-meta-item" style={{color: '#57534e'}}><MapPin size={14} /> {business.distance_km} km</span>
          )}
        </div>
      </div>
      
      <div className="cc-right">
        {business.is_closed_today ? (
          ...
          <div style={{ textAlign: "right", display: "flex", flexDirection: "column", gap: "2px" }}>
            ...
            <div className="cc-wait-val" style={{color: '#57534e'}}>
              <Clock size={16} /> {Math.max((business.waiting_count || 0) * 5, 10)} mins
            </div>
            <div style={{ fontSize: "11px", color: "#64748b" }}>
              {business.waiting_count || 0} waiting
            </div>
          </div>
        )}
    ...
    <>
      ...
      {showPopup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ background: "#f8fafc", border: "1px solid rgba(87,83,78,0.2)", borderRadius: "16px", padding: "32px", width: "90%", maxWidth: "400px", position: "relative", textAlign: "center", boxShadow: "0 20px 40px rgba(0,0,0,0.5)" }}>
            <button 
            ...
            <h3 style={{ fontSize: "20px", fontWeight: 700, color: "#fff", marginBottom: "12px" }}>Invite Your Admin</h3>
            <p style={{ fontSize: "14px", color: "#d6d3d1", marginBottom: "24px", lineHeight: 1.5 }}>
              Share this link with your business admin so they can register for free and set up digital queuing.
            </p>
            <div style={{ display: "flex", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 8px 8px 16px", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.1)" }}>
              ...
              <button 
                onClick={handleCopy}
                style={{ background: "#f59e0b", color: "#fff", border: "none", borderRadius: "8px", padding: "10px 20px", fontSize: "14px", fontWeight: 700, cursor: "pointer", transition: "all 0.2s", opacity: copied ? 0.8 : 1 }}
              >
...
export default function FindOtherClient({ initialClinics, initialQ, initialCity, initialSpecialty, cities, specialties }) {
  ...
  const fetchClinics = useCallback(async (q, city, spec, status) => {
    ...
    try {
      ...
      params.set('vertical', 'business')   // ← strict vertical isolation
      if (q) params.set('q', q)
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      router.replace(`/find-business${params.toString() ? '?' + params.toString() : ''}`, { scroll: false })
    }, 300)
  ...
  const fetchNearby = useCallback(async (lat, lng) => {
    try {
      const res = await fetch(`/api/clinics/nearby?lat=${lat}&lng=${lng}&radius=50000&vertical=business`)
      const json = await res.json()
      setNearbyClinics(json.clinics || [])
      setIsNearbyMode(true)
    } catch {
      setNearbyClinics([])
      setGpsError('Could not fetch nearby businesss. Please try again.')
    } finally {
      setGpsLoading(false)
    }
  }, [])
  ...
  const allSpecialties = specialties && specialties.length ? specialties : ['Retail', 'Services', 'Events', 'Repairs', 'Consulting']
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Plus Jakarta Sans', sans-serif; background: #0a0f1e; color: #e2e8f0; }
        
        .find-nav { padding: 20px 24px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); position: sticky; top: 0; z-index: 100; background: #0a0f1e; }
        ...
        .find-nav-btn { 
          ...
          box-shadow: 0 0 0 rgba(87,83,78,0);
        }
        .find-nav-btn:hover { 
          background: rgba(87,83,78,0.15); 
          transform: translateY(-2px) scale(1.02);
          box-shadow: 0 4px 16px rgba(87,83,78,0.4);
          color: #78716c;
          border-color: #78716c;
        }
        ...
        .page-sub { color: #64748b; font-size: 16px; margin-bottom: 40px; }
        ...
        .search-input:focus { border-color: #57534e; }
        
        .btn-search { padding: 0 32px; height: 53px; background: #57534e; color: #fff; border: none; border-radius: 12px; font-weight: 700; font-size: 16px; cursor: pointer; transition: background 0.2s; }
        .btn-search:hover { background: #78716c; }
        
        .filters-row { display: flex; align-items: center; gap: 12px; margin-bottom: 28px; flex-wrap: wrap; }
        
        .btn-near-me { padding: 8px 16px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; transition: all 0.2s; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); color: #e2e8f0; }
        .btn-near-me:hover { background: rgba(255,255,255,0.08); }
        .btn-near-me:disabled { opacity: 0.7; cursor: not-allowed; }
        .btn-near-me.active { background: rgba(87,83,78,0.1); border-color: #57534e; color: #57534e; }
        ...
        .filter-select { padding: 8px 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); font-size: 13px; color: #e2e8f0; background: rgba(255,255,255,0.03); outline: none; cursor: pointer; }
        .filter-select option { background: #f8fafc; color: #fff; }
        ...
        .clinic-card:hover { border-color: rgba(87,83,78,0.3); background: rgba(255,255,255,0.04); }
        .clinic-card.selected { border-color: #57534e; background: rgba(87,83,78,0.05); }
        
        .cc-title { font-size: 20px; font-weight: 700; color: #fff; margin-bottom: 8px; }
        .cc-meta { display: flex; gap: 16px; color: #64748b; font-size: 14px; flex-wrap: wrap; }
        .cc-meta-item { display: flex; align-items: center; gap: 4px; }
        
        .cc-right { display: flex; alignItems: center; gap: 24px; }
        .cc-wait-label { font-size: 13px; color: #64748b; margin-bottom: 4px; }
        ...
        .cc-btn-active { background: #57534e; color: #fff; }
        .cc-btn-active:hover { background: #78716c; }
        .cc-btn-disabled { background: rgba(255,255,255,0.1); color: #64748b; cursor: not-allowed; }
        
        .empty-state-overlay { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(15,23,42,0.85); padding: 32px; border-radius: 20px; backdrop-filter: blur(8px); display: flex; flex-direction: column; align-items: center; width: 340px; text-align: center; z-index: 10; border: 1px solid rgba(255,255,255,0.1); }
        .empty-icon-wrap { width: 64px; height: 64px; background: rgba(87,83,78,0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 20px; }
        .btn-invite { background: transparent; border: 1px solid #57534e; color: #57534e; padding: 10px 24px; border-radius: 8px; font-size: 13px; font-weight: 700; cursor: pointer; margin-top: 8px; }
        ...
        @media (max-width: 768px) {
          ...
          .mobile-view-btn { flex: 1; text-align: center; padding: 8px; font-size: 14px; font-weight: 600; cursor: pointer; border-radius: 6px; color: #64748b; transition: all 0.2s; }
      ...
      <nav className="find-nav">
        ...
        <a href="/business-login" className="find-nav-btn">
          Business Login
        </a>
      </nav>
      ...
      <div className={`layout-container ${activeView === 'map' ? 'map-active' : ''}`}>
        <div className="left-col">
          <h1 className="page-title">Find a TokenPe Business</h1>
          <p className="page-sub">Search by business name, department, or city to join a waitlist remotely.</p>
          ...
          <div className="search-row">
            <div className="search-input-wrap">
              ...
              <input 
                ...
                type="text" 
                placeholder="Search e.g., 'Electronics in Mumbai'" 
                value={query} 
          ...
          <div className="filters-row">
            ...
            >
              <MapPin size={14} color={isNearbyMode ? "#57534e" : "#64748b"} /> 
          ...
          <div className="clinic-list">
            {(loading || gpsLoading) ? (
              ...
              <>
                ...
                {displayedClinics.map(business => (
                  <BusinessCard 
                    key={business.id || business.code} 
                    business={business} 
                    isNearby={isNearbyMode} 
                    isSelected={selectedClinic && (selectedClinic.id === business.id || selectedClinic.code === business.code)}
                    onClick={() => {
                      setSelectedClinic(business)
              ...
              <div style={{ padding: '40px', textAlign: 'center', color: '#64748b', background: "rgba(255,255,255,0.01)", borderRadius: "16px", border: "1px dashed rgba(255,255,255,0.1)" }}>
                No businesss found matching your search.
              </div>
            )}
          </div>
        </div>
