...
export async function POST(req) {
    try {
        ...
        // Check for existing restaurant with same phone number
        ...
        if (error) {
            return Response.json({ success: false, message: 'Failed to create restaurant. Phone may already be registered.' }, { status: 500 })
        }
        ...
        cookieStore.set('tokenpe_restaurant_session', token, {
            httpOnly: true,
        ...
        return Response.json({ success: true, restaurant: data }, { status: 200 })

    } catch (error) {
        console.error('[Restaurant Register API Error]', error)
