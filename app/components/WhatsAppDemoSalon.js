...
const DEFAULT_FLOW = [
  { side: "pat", type: "text", time: "9:56 AM", text: "JOIN BELLESALON", delay: 1000 },
  ...
  return (
    <div style={S.card}>
      <div style={{ padding: "8px 10px 6px" }}>
        <div style={{ fontSize: "12.5px", fontWeight: 700, color: "#111b21", marginBottom: "4px" }}>{m.title || "✂️ Welcome to Belle Salon & Spa!"}</div>
...
function JoinReply({ msg }) {
  ...
  return (
    <div style={{ ...S.bubble, ...S.pat }} className="wa-pat-tail">
      <LRQuote title={m.title || "✂️ Welcome to Belle Salon & Spa!"} body={m.quoteBody || "You are about to join the digital queue..."} />
  ...
  return (
    <div style={S.card}>
      <div style={{ padding: "8px 10px 6px" }}>
        ...
        <div style={{ fontSize: "11px", color: "#111b21", lineHeight: 1.45 }}>{fmt(m.body || "How was your visit at Belle Salon & Spa?\nYour feedback helps us improve! 🙏")}</div>
        <em style={{ fontSize: "9.5px", color: "#8696a0", fontStyle: "italic", marginTop: "4px", display: "block" }}>Powered by TokenPe</em>
      </div>
...
function RatingReply({ msg }) {
  ...
  return (
    <div style={{ ...S.bubble, ...S.pat }} className="wa-pat-tail">
      <LRQuote title={m.title || "Rate Your Visit"} body={m.quoteBody || "How was your visit at Belle Salon & Spa?..."} />
      <div style={{ fontSize: "12px", marginTop: "2px" }}>{m.text}</div>
      <TS time={msg.time} pat />
    </div>
  );
...
export default function WhatsAppDemoSalon({ flow }) {
  ...
  return (
    <>
      <style>{`
        ...
        .wa-wrapper { display: flex; justify-content: center; align-items: center; padding: 12px 8px 24px; position: relative; width: 100%; box-sizing: border-box; overflow: visible; }
        .wa-wrapper::after {
          content: '';
          position: absolute;
          bottom: 0;
